...
@Service
public class SubscribeService implements ReplyCallbackQuery{
    ...
    public BotApiMethod<?> reply(long chatId, int messageId, String callbackData) {
        ...
        try {
            userService.updateUserAddConcert(chatId, concertId);
        } catch (DataNotFoundException e) {
            return new EditMessageText()
                    .setChatId(chatId)
                    .setMessageId(messageId)
                    .setText("Concert already passed. Impossible to subscribe")
                    .setReplyMarkup(null);
        }
    ...
    private EditMessageReplyMarkup editReplyMarkup(int messageId, long chatId, long concertId){
        ...
        rowInline.add(new InlineKeyboardButton().setText("Subscribed " + Emoji.SUBSCRIBE).setCallbackData(QueryType.UNSUBSCRIBE + " " + concertId));
